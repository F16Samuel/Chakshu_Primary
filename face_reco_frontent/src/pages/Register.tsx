...
// Define the unified FastAPI endpoint for registration
const REGISTRATION_API_URL = `${BACKEND_URL}/register`;
...
export default function Register() {
  ...
  const [formData, setFormData] = useState({
    ...
    facePhoto1: [] as File[],
    facePhoto2: [] as File[],
    // webcamPhoto: null as File | null, // Removed as we're now using facePhoto1/2 for webcam captures
  });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!isFormValid) { // Use the pre-calculated isFormValid
      ...
      // FastAPI's register endpoint expects multiple face photos directly as 'face_photo_1', 'face_photo_2' etc.
      // However, the current backend `main.py` only expects `webcam_photo` if provided,
      // and relies on `extract_face_encodings_from_bytes` for face detection.
      // To align with the backend, we should send the face photos as separate fields.
      // The `main.py` `register` endpoint currently expects `webcam_photo` as a single file.
